...
select us.nombre,ug.us_id from usuarios us,usuario_grupo ug
where us.id=ug.us_id
and id IN(SELECT us_id FROM usuario_grupo where ug.gr_id=1)

select gr.nombre, count (ug.us_id) AS total_usuario_grupo 
...
select us.nombre,ug.us_id from usuarios us, usuario_grupo ug
where us.id=ug.us_id
and id IN(SELECT us_id FROM usuario_grupo WHERE gr_id=2);

select gr.nombre,MAX(ug.us_id) AS max_usuarios, MIN(ug.us_id) AS min_usuarios
from grupo gr, usuario_grupo ug
where gr.id=ug.gr_id
group by gr.nombre
...
where us.id=ug.us_id
and gr.id=ug.gr_id
and gr.fecha_creacion between '08/03/2020' and '08/03/2022'

select us.nombre,ug.us_id from usuarios us, usuario_grupo ug
where us.id=ug.us_id
and id IN(SELECT us_id FROM usuario_grupo WHERE gr_id=3)

select gr.descripcion,COUNT(ug.us_id) AS total_usuarios
from grupo gr, usuario_grupo ug
where gr.id=ug.gr_id
